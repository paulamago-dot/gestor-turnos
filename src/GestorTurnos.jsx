...
const THEME = {
  light: {
    ...
    rowBrd:   "#e0e7ff",
    // extra
    navBg:    "linear-gradient(180deg,#4338ca 0%,#312e81 100%)",
    accent:   "#fbbf24",
    subBg:    "#f5f3ff",
    subBrd:   "#ddd6fe",
    cardHead: "#1e1b4b",
    btnPri:   "linear-gradient(135deg,#6366f1,#8b5cf6)",
    modalBg:  "#ffffff",
    modalBrd: "#c7d2fe",
    navText:  "#c7d2fe",
  },
  dark: {
    ...
    rowBrd:   "#1e3a8a",
    // extra
    navBg:    "linear-gradient(180deg,#020712 0%,#060f22 100%)",
    accent:   "#fbbf24",
    subBg:    "rgba(30,58,138,0.28)",
    subBrd:   "#1e3a8a",
    cardHead: "#38bdf8",
    btnPri:   "linear-gradient(135deg,#1d4ed8,#7c3aed)",
    modalBg:  "#0d1630",
    modalBrd: "#1e3a8a",
    navText:  "#7dd3fc",
  },
...
// ─── ABSENCE MODAL ────────────────────────────────────────────────────────────
function AbsenceModal({ date, shift, gIdx, workingCount, onConfirm, onClose, T }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.modalBg,borderRadius:20,
        padding:24,width:"100%",maxWidth:400,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",
        border:`1px solid ${T.modalBrd}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,fontSize:17,fontWeight:800,color:T.textPri}}>Solicitar día libre</h3>
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:T.textMut,lineHeight:1}}>×</button>
        </div>
        <div style={{padding:"8px 12px",background:T.subBg,borderRadius:10,marginBottom:16,fontSize:13,color:T.textSec,border:`1px solid ${T.subBrd}`}}>
          <b style={{color:T.textPri}}>{date.getDate()} de {MONTHS[date.getMonth()]}</b>
          &nbsp;· Turno <ShiftChip shift={shift} small/>
          &nbsp;· <b style={{color: workingCount<3?"#ef4444":"#22c55e"}}>{workingCount} trabajando</b>
        </div>
            ...
            return (
              <button key={a.id} onClick={()=>!isBlocked&&setReason(a.id)}
                style={{padding:"10px 12px",borderRadius:10,textAlign:"left",cursor:isBlocked?"not-allowed":"pointer",
                  border:`2px solid ${reason===a.id?a.c:T.cardBrd}`,
                  background:reason===a.id?a.c+"22":isBlocked?T.subBg:T.cardBg,
                  opacity:isBlocked?0.5:1,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span style={{fontSize:13,fontWeight:600,color:isBlocked?T.textMut:reason===a.id?a.c:T.textPri}}>{a.l}</span>
                {isBlocked&&<span style={{fontSize:10,color:"#ef4444"}}>min. no cubierto</span>}
                {!a.r&&<span style={{fontSize:10,color:"#22c55e"}}>siempre permitido</span>}
              </button>
            );
          })}
        </div>
        <button onClick={()=>!blocked&&onConfirm(reason)}
          style={{width:"100%",padding:"11px 0",borderRadius:12,border:"none",fontWeight:700,fontSize:14,
            ...
            background:blocked?T.subBg:GC[gIdx],color:blocked?T.textMut:"white"}}>
...
function GeneralModal({ date, shift, users, absences, swaps, onClose, T }) {
  ...
  for (let g = 0; g < 5; g++) {
    ...
    gUsers.forEach(u => {
      ...
      if (abs) {
        ...
        if (sub) rows.push({ type:"working", worker:sub, originalUser:u, g, atype:null, isSubstitute:true, subG: sub.gId });
      } else {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.modalBg,borderRadius:20,
        ...
        boxShadow:"0 20px 60px rgba(0,0,0,0.5)",border:`1px solid ${T.modalBrd}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
          <div>
            <h3 style={{margin:0,fontSize:17,fontWeight:800,color:T.textPri}}>
              {date.getDate()} de {MONTHS[date.getMonth()]} {date.getFullYear()}
            </h3>
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4}}>
              <ShiftChip shift={shift}/>
              <span style={{fontSize:13,color:T.textSec}}>{SI[shift].l} · {SI[shift].h}</span>
              <span style={{fontSize:12,fontWeight:700,color:working.length>=3?"#22c55e":"#ef4444"}}>
        ...
        {working.length > 0 && (
          <div style={{marginBottom:14}}>
            <p style={{fontSize:11,fontWeight:700,color:T.textMut,margin:"0 0 6px",textTransform:"uppercase",letterSpacing:0.5}}>
            ...
            {working.map((row, i) => {
              ...
              return (
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",
                  ...
                  background: row.isSubstitute ? "rgba(34,197,94,0.12)" : T.subBg,
                  border: row.isSubstitute ? "1px solid rgba(34,197,94,0.3)" : `1px solid ${T.subBrd}`}}>
                  <Avatar name={row.worker.name} size={30} color={GC[displayG]}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                      <p style={{margin:0,fontSize:13,fontWeight:700,color:T.textPri,
                    ...
                    {row.isSubstitute && row.originalUser && (
                      <div style={{display:"flex",alignItems:"center",gap:4,marginTop:3}}>
                        <span style={{fontSize:10,color:"#22c55e",fontWeight:600}}>↔ cambio con</span>
                        <span style={{fontSize:10,color:T.textSec}}>{row.originalUser.name}</span>
                        <GuardChip gIdx={row.originalUser.gId}/>
                      </div>
                    )}
                  </div>
                  {row.isSubstitute && (
                    <span style={{fontSize:9,background:"rgba(34,197,94,0.2)",color:"#22c55e",
        ...
        {absent.length > 0 && (
          <div>
            ...
            {absent.map((row, i) => (
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",
                borderRadius:10,marginBottom:4,background:row.atype?.c+"22",
                border:`1px solid ${row.atype?.c}44`}}>
                <Avatar name={row.worker.name} size={30} color={GC[row.g]}/>
                <div style={{flex:1,minWidth:0}}>
                  <p style={{margin:0,fontSize:13,fontWeight:700,color:T.textPri}}>{row.worker.name}</p>
                  <div style={{display:"flex",gap:6,alignItems:"center",marginTop:2}}>
                    <GuardChip gIdx={row.g}/>
                    {row.atype && (
                      <span style={{fontSize:10,color:row.atype.c,background:row.atype.c+"25",
...
// ─── SWAP DETAIL MODAL ────────────────────────────────────────────────────────
function SwapDetailModal({ swap, users, currentUserId, onCancel, onClose, cancelLabel, T }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.modalBg,borderRadius:20,
        padding:24,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",
        border:`1px solid ${T.modalBrd}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <h3 style={{margin:0,fontSize:16,fontWeight:800,color:T.textPri}}>Detalle del cambio</h3>
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:T.textMut,lineHeight:1}}>×</button>
        </div>

        <div style={{background:T.subBg,borderRadius:12,padding:"10px 14px",marginBottom:14,
          display:"flex",alignItems:"center",gap:10,border:`1px solid ${T.subBrd}`}}>
          <ShiftChip shift={swap.fromShift}/>
          <span style={{fontSize:14,fontWeight:700,color:T.textPri}}>{swap.fromDate}</span>
          <span style={{fontSize:12,color:T.textSec}}>{SI[swap.fromShift]?.h}</span>
        </div>

        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:8,alignItems:"center",marginBottom:14}}>
          <div style={{background:T.subBg,borderRadius:10,padding:"8px 10px",textAlign:"center",border:`1px solid ${T.subBrd}`}}>
            <p style={{margin:0,fontSize:10,color:T.textMut,fontWeight:600}}>Cede turno</p>
            <p style={{margin:"4px 0 2px",fontSize:12,fontWeight:700,color:T.textPri}}>{fromUser?.name?.split(" ")[0]}</p>
            <GuardChip gIdx={iFrom}/>
          </div>
          <span style={{fontSize:18,color:T.accent,textAlign:"center"}}>↔</span>
          <div style={{background:T.subBg,borderRadius:10,padding:"8px 10px",textAlign:"center",border:`1px solid ${T.subBrd}`}}>
            <p style={{margin:0,fontSize:10,color:T.textMut,fontWeight:600}}>Cubre turno</p>
            <p style={{margin:"4px 0 2px",fontSize:12,fontWeight:700,color:T.textPri}}>{toUser?.name?.split(" ")[0]}</p>
            <GuardChip gIdx={iTo}/>
          </div>
        </div>

        <div style={{borderTop:`1px solid ${T.rowBrd}`,paddingTop:12,marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            <span style={{fontSize:11,color:T.textMut}}>Solicitado</span>
            <span style={{fontSize:11,color:T.textPri,fontWeight:600}}>{fmt(swap.proposedAt)}</span>
          </div>
          <div style={{display:"flex",justifyContent:"space-between"}}>
            <span style={{fontSize:11,color:T.textMut}}>Aceptado</span>
            <span style={{fontSize:11,color:"#22c55e",fontWeight:600}}>{fmt(swap.acceptedAt)}</span>
          </div>
        </div>

        <button onClick={onCancel}
          style={{width:"100%",padding:"10px",borderRadius:12,border:"2px solid #ef444466",
...
// ─── GUARD SHIFT MODAL ───────────────────────────────────────────────────────
function GuardShiftModal({ date, shift, gIdx, users, absences, swaps, extraShifts, supplements, onClose, T }) {
  const dateStr = ds(date);
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.modalBg,borderRadius:20,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              <GuardChip gIdx={gIdx}/>
              <span style={{fontSize:12,color:T.textSec}}>{SI[shift].h}</span>
              <span style={{fontSize:12,fontWeight:700,color:low?"#ef4444":"#22c55e"}}>
                {working} trabajando{low?" ⚠️":""}
              </span>
              {supp&&<span style={{fontSize:11,color:"#22c55e",fontWeight:700}}>+{supp.amount}€</span>}
            </div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:T.textMut,lineHeight:1}}>×</button>
        </div>
        {rows.map(({u,abs,swA,ex,at,coverer})=>{
          ...
          const bg   = isLibre?"rgba(74,222,128,0.18)":isForceMajeure?"rgba(248,113,113,0.18)":swA?T.subBg:"rgba(34,197,94,0.12)";
          const bdr  = isLibre?"#22c55e":isForceMajeure?"#ef4444":swA?T.textMut:"#22c55e";
          const label= abs?at?.l:swA?"Cambiado":ex?"Extra":"Trabajando";
          const lc   = isLibre?"#22c55e":isForceMajeure?"#ef4444":swA?T.textMut:"#22c55e";
          return (
            <div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                {swA&&coverer&&(
                  <p style={{margin:"2px 0 0",fontSize:11,color:T.textSec}}>
                    Cubierto por <b>{coverer.name.split(" ")[0]}</b> · G{(coverer.gId??0)+1}
                  </p>
                )}
              </div>
              <span style={{fontSize:10,fontWeight:700,background:lc+"25",color:lc,
        ...
        {swIn.length>0&&(
          <div style={{marginTop:8,borderTop:`1px solid ${T.rowBrd}`,paddingTop:10}}>
            <p style={{fontSize:11,fontWeight:700,color:T.textMut,margin:"0 0 6px",textTransform:"uppercase",letterSpacing:0.5}}>
            ...
            {swIn.map(s=>{
              ...
              return sub?(
                <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",
                  borderRadius:10,marginBottom:6,background:"rgba(34,197,94,0.12)",borderLeft:"3px solid #22c55e"}}>
                  <Avatar name={sub.name} size={30} color={GC[sub.gId??0]}/>
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{margin:0,fontSize:13,fontWeight:700,color:T.textPri}}>{sub.name}</p>
                    <p style={{margin:"2px 0 0",fontSize:11,color:T.textSec}}>
                      Cubre a <b>{orig?.name?.split(" ")[0]}</b> · G{(sub.gId??0)+1}
                    </p>
                  </div>
                  <span style={{fontSize:10,fontWeight:700,background:"rgba(34,197,94,0.2)",color:"#22c55e",
...
// ─── OFFER BUILDER MODAL ────────────────────────────────────────────────────
function OfferBuilderModal({ req, fromUser, currentUser, users, userWorksShift, checkConflict, onSubmit, onClose, T }) {
  const [options, setOptions] = useState([{date:"",shift:"M"}]);
  const inp = {border:`1px solid ${T.inputBrd}`,borderRadius:10,padding:"8px 12px",fontSize:13,outline:"none",background:T.inputBg,color:T.textPri};
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.modalBg,borderRadius:20,
        ...
        boxShadow:"0 20px 60px rgba(0,0,0,0.5)",border:`1px solid ${T.modalBrd}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,fontSize:16,fontWeight:800,color:T.textPri}}>Hacer oferta de cambio</h3>
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:T.textMut,lineHeight:1}}>×</button>
        </div>

        <div style={{background:T.subBg,borderRadius:12,padding:"10px 14px",marginBottom:16,border:`1px solid ${T.subBrd}`}}>
          <p style={{margin:0,fontSize:11,color:T.textMut,fontWeight:600}}>SOLICITUD DE {fromUser?.name?.toUpperCase()}</p>
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4}}>
            <ShiftChip shift={req.fromShift}/>
            <span style={{fontSize:14,fontWeight:700,color:T.textPri}}>{req.fromDate}</span>
            <span style={{fontSize:12,color:T.textSec}}>{SI[req.fromShift]?.h}</span>
          </div>
          <p style={{margin:"6px 0 0",fontSize:12,color:T.textSec}}>
            Tú cubrirás este turno. A cambio, ofrece los días que quieres que te cubran:
          </p>
        </div>

        <p style={{fontSize:12,fontWeight:700,color:T.textPri,margin:"0 0 10px"}}>
          ...
          return (
            <div key={i} style={{display:"flex",gap:6,alignItems:"center",marginBottom:8}}>
              <input type="date" value={opt.date} onChange={e=>updOpt(i,"date",e.target.value)}
                style={{...inp,flex:1,borderColor:errMsg?"#ef4444":T.inputBrd}}/>
              ...
              {options.length>1&&<button onClick={()=>remOpt(i)}
                style={{width:28,height:28,borderRadius:"50%",background:"rgba(239,68,68,0.15)",border:"none",cursor:"pointer",color:"#ef4444",fontWeight:700,fontSize:14,flexShrink:0}}>×</button>}
              {isValid&&<span style={{fontSize:11,color:"#22c55e",flexShrink:0}}>✓</span>}
              {errMsg&&<span style={{fontSize:10,color:"#ef4444",flexShrink:0,maxWidth:80,lineHeight:1.2}}>{errMsg}</span>}
            </div>
          );
        })}
        ...
        {options.length<5&&(
          <button onClick={addOpt}
            style={{fontSize:12,color:T.textSec,background:T.subBg,border:`1px dashed ${T.subBrd}`,
              borderRadius:10,padding:"6px 12px",cursor:"pointer",fontWeight:600,marginBottom:16,width:"100%"}}>
            + Añadir otra opción
          </button>
        )}

        {!canSubmit&&<p style={{fontSize:11,color:"#ef4444",margin:"0 0 10px"}}>
        ...
        <button onClick={()=>canSubmit&&onSubmit(validOptions)}
          ...
          style={{width:"100%",padding:"11px",borderRadius:12,border:"none",fontWeight:700,fontSize:14,
            ...
            background:canSubmit?T.btnPri:T.subBg,color:canSubmit?"white":T.textMut}}>
...
function ExtraShiftForm({ users, onAdd, isMobile, inp, T }) {
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"1fr 1fr 80px 120px",gap:8,marginBottom:12}}>
      ...
      <button onClick={()=>{if(!uid||!date)return; onAdd({id:Date.now(),userId:Number(uid),date,shift,compensation:comp}); setUid("");setDate("");}}
        style={{gridColumn:"1/-1",background:T.btnPri,color:"white",border:"none",borderRadius:10,padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer"}}>
        Asignar turno extra
      </button>
    </div>
  );
...
function SupplementForm({ onAdd, isMobile, inp, T }) {
...
function MainApp({ currentUser, onLogout,
  ...
  const sidebar = {width:210,background:T.navBg,
      ...
      <div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <button onClick={prevMonth} style={{width:isMobile?44:32,height:isMobile?44:32,borderRadius:"50%",background:T.subBg,border:`1px solid ${T.subBrd}`,cursor:"pointer",fontSize:isMobile?20:16,display:"flex",alignItems:"center",justifyContent:"center",color:T.textPri}}>‹</button>
            <span style={{fontWeight:700,color:T.textPri,width:isMobile?150:130,textAlign:"center",fontSize:isMobile?15:14}}>{MONTHS[calM]} {calY}</span>
            <button onClick={nextMonth} style={{width:isMobile?44:32,height:isMobile?44:32,borderRadius:"50%",background:T.subBg,border:`1px solid ${T.subBrd}`,cursor:"pointer",fontSize:isMobile?20:16,display:"flex",alignItems:"center",justifyContent:"center",color:T.textPri}}>›</button>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:3}}>
          {WDAYS.map(d=>(
            <div key={d} style={{textAlign:"center",fontSize:11,fontWeight:700,color:T.textMut,padding:"4px 0"}}>{d}</div>
          ))}
        </div>
          ...
          {Array(days).fill(null).map((_,i)=>{
            ...
            return (
              <div key={day} style={{border:isToday?`2px solid #f59e0b`:`1px solid ${T.cellBrd}`,
                ...
                <div style={{fontSize:isMobile?12:11,fontWeight:700,color:isToday?"#f59e0b":T.textMut,marginBottom:isMobile?3:4}}>{day}</div>
        ...
        <div style={{display:"flex",gap:16,marginTop:16,flexWrap:"wrap"}}>
          {Object.entries(SI).map(([k,v])=>(
            <div key={k} style={{display:"flex",alignItems:"center",gap:5}}>
              ...
              <span style={{fontSize:11,color:T.textSec}}>{k} – {v.l}</span>
            </div>
          ))}
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <div style={{width:10,height:10,borderRadius:3,background:"rgba(239,68,68,0.2)",borderLeft:"3px solid #ef4444"}}/>
            <span style={{fontSize:11,color:T.textSec}}>Crítico (&lt;3)</span>
          </div>
        </div>
      </div>
    );
      ...
      <div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            {guardROMode&&<span style={{fontSize:11,background:T.subBg,color:T.textSec,padding:"2px 8px",borderRadius:20,fontWeight:600,border:`1px solid ${T.subBrd}`}}>Solo lectura</span>}
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <button onClick={prevMonth} style={{width:isMobile?44:30,height:isMobile?44:30,borderRadius:"50%",background:T.subBg,border:`1px solid ${T.subBrd}`,cursor:"pointer",fontSize:isMobile?20:15,display:"flex",alignItems:"center",justifyContent:"center",color:T.textPri}}>‹</button>
            <span style={{fontWeight:700,color:T.textPri,width:isMobile?130:110,textAlign:"center",fontSize:isMobile?14:13}}>{MONTHS[calM]} {calY}</span>
            <button onClick={nextMonth} style={{width:isMobile?44:30,height:isMobile?44:30,borderRadius:"50%",background:T.subBg,border:`1px solid ${T.subBrd}`,cursor:"pointer",fontSize:isMobile?20:15,display:"flex",alignItems:"center",justifyContent:"center",color:T.textPri}}>›</button>
          </div>
        </div>

        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:3}}>
          {WDAYS.map(d=><div key={d} style={{textAlign:"center",fontSize:11,fontWeight:700,color:T.textMut,padding:"4px 0"}}>{d}</div>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
    ...
    return (
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:isMobile?2:3,marginBottom:3}}>
          {WDAYS.map(d=><div key={d} style={{textAlign:"center",fontSize:isMobile?12:11,fontWeight:700,color:T.textMut,padding:"4px 0"}}>{d}</div>)}
        </div>
      ...
      <div>
        ...
        {!isBlocked&&(
          <div style={{...card,marginBottom:20}}>
            <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:700,color:T.cardHead}}>Solicitar cobertura de turno</h3>
              ...
              return (
                <div key={req.id} style={{...card,marginBottom:10,borderLeft:`4px solid #f59e0b`}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                    <ShiftChip shift={req.fromShift}/>
                    <span style={{fontSize:14,fontWeight:700,color:T.textPri}}>{req.fromDate}</span>
                    <span style={{fontSize:12,color:T.textSec}}>{req.note}</span>
                  </div>
                    ...
                    return (
                      <div key={offer.id} style={{background:"#fef9ee",borderRadius:12,padding:"10px 14px",marginBottom:8,border:"1px solid #fde68a"}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                          ...
                          <span style={{fontSize:13,fontWeight:700,color:T.textPri}}>{offerer?.name}</span>
                          <GuardChip gIdx={offerer?.gId??0}/>
                          <span style={{fontSize:11,color:T.textMut}}>propone {offer.options.length} opción{offer.options.length>1?"es":""}</span>
                        </div>
          ...
          <div style={{marginBottom:20}}>
            ...
            {myRequests.filter(r=>r.status==="open").map(req=>(
              <div key={req.id} style={{...card,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6,padding:"12px 16px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <ShiftChip shift={req.fromShift}/>
                  <span style={{fontSize:13,fontWeight:600,color:T.textPri}}>{req.fromDate}</span>
                  {req.toId&&<span style={{fontSize:11,color:T.textSec}}>→ {users.find(u=>u.id===req.toId)?.name?.split(" ")[0]}</span>}
                  {req.note&&<span style={{fontSize:12,color:T.textMut}}>"{req.note}"</span>}
                </div>
                <button onClick={()=>setSwapRequests(rs=>rs.map(r=>r.id===req.id?{...r,status:"cancelled"}:r))}
        ...
        {openForMe.length===0?(
          <div style={{textAlign:"center",padding:"40px 0",color:T.textMut,fontSize:14}}>No hay solicitudes disponibles</div>
        ):openForMe.map(req=>{
            ...
            <div key={req.id} style={{...card,marginBottom:8,padding:"14px 16px"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div>
                    <p style={{margin:0,fontSize:13,fontWeight:700,color:T.textPri}}>{fromUser?.name}</p>
                    <div style={{display:"flex",gap:6,alignItems:"center",marginTop:3}}>
                      <GuardChip gIdx={gI}/><ShiftChip shift={req.fromShift} small/>
                      <span style={{fontSize:11,color:T.textMut}}>{req.fromDate}</span>
                      {req.note&&<span style={{fontSize:11,color:T.textMut}}>· {req.note}</span>}
                    </div>
                    {req.offers.length>0&&<p style={{margin:"3px 0 0",fontSize:10,color:T.textMut}}>{req.offers.filter(o=>o.status==="pending").length} oferta(s) pendiente(s)</p>}
                  </div>
                </div>
        ...
        {offerModal&&(()=>{
          ...
          return (
            <OfferBuilderModal
              ...
              currentUser={currentUser} users={users} T={T}
    ...
    return (
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:20}}>
          {[
            {l:"Total funcionarios",v:allWorkers.length,bg:"linear-gradient(135deg,#4338ca,#6366f1)",c:"white"},
        ...
        <div style={{...card,marginBottom:16}}>
          <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:700,color:T.cardHead}}>Ausencias por motivo</h3>
          {AT.map(a=>{
            ...
            return (
              <div key={a.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                ...
                <span style={{fontSize:13,color:T.textPri,flex:1}}>{a.l}</span>
                <span style={{fontSize:13,fontWeight:700,color:T.textPri,minWidth:20,textAlign:"right"}}>{cnt}</span>
                <div style={{width:80,height:6,background:T.subBg,borderRadius:3,flexShrink:0}}>
                  <div style={{width:`${pct}%`,height:"100%",background:a.c,borderRadius:3}}/>
                </div>
              </div>
            );
          })}
          {absences.length===0&&<p style={{fontSize:13,color:T.textMut}}>Sin ausencias registradas</p>}
        </div>

        {/* Turnos extra */}
        <div style={{...card,marginBottom:16}}>
          <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:700,color:T.cardHead}}>Turnos extra</h3>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div style={{background:"linear-gradient(135deg,#d97706,#b45309)",borderRadius:12,padding:"12px 16px"}}>
              <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.8)"}}>Compensación económica</p>
              <p style={{margin:"4px 0 0",fontSize:24,fontWeight:900,color:"white"}}>{economicExtras.length}</p>
            </div>
            <div style={{background:"linear-gradient(135deg,#059669,#0284c7)",borderRadius:12,padding:"12px 16px"}}>
              <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.8)"}}>Compensación en días</p>
              <p style={{margin:"4px 0 0",fontSize:24,fontWeight:900,color:"white"}}>{daysExtras.length}</p>
            </div>
          </div>
        </div>
        ...
        {!isAdmin&&myG!==null&&(
          <div style={card}>
            <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:700,color:T.cardHead}}>Saldo de días – Guardia {myG+1}</h3>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                <thead>
                  <tr style={{borderBottom:`1px solid ${T.cardBrd}`}}>
                    <th style={{textAlign:"left",padding:"6px 8px",color:T.textSec,fontWeight:600}}>Funcionario</th>
                ...
                <tbody>
                  {users.filter(u=>u.gId===myG&&u.id!==0).map(u=>(
                    <tr key={u.id} style={{borderBottom:`1px solid ${T.rowBrd}`}}>
                      <td style={{padding:"6px 8px",fontWeight:600,color:T.textPri}}>{u.name.split(" ")[0]}</td>
                      {AT.filter(a=>a.r).map(a=>{
                        ...
                        return <td key={a.id} style={{textAlign:"center",padding:"6px 4px"}}>
                          <span style={{fontWeight:700,color:rem<2?"#ef4444":T.textPri}}>{rem}</span>
                          <span style={{fontSize:10,color:T.textMut}}>/{bal}</span>
                        </td>;
                      })}
                    </tr>
                  ))}
  ...
  const renderAdmin = () => {
    ...
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
        {[{id:"pending",l:`Solicitudes${pendingCnt>0?` (${pendingCnt})`:""}`},{id:"users",l:"Usuarios"},{id:"balances",l:"Saldos"},{id:"extra",l:"Turnos Extra"},{id:"supplements",l:"Suplementos"}].map(t=>(
          <button key={t.id} onClick={()=>setAdminTab(t.id)}
            style={{padding:"7px 16px",borderRadius:10,border:`1px solid ${T.cardBrd}`,fontWeight:700,fontSize:12,cursor:"pointer",
              background:adminTab===t.id?T.btnPri:T.subBg,color:adminTab===t.id?"white":T.textSec}}>
            {t.l}
          </button>
        ))}
      </div>
      ...
      {adminTab==="pending"&&(
        pending.length===0?(
          <div style={{textAlign:"center",padding:"40px 0",color:T.textMut}}>No hay solicitudes pendientes</div>
        ):pending.map(u=>(
          <div key={u.id} style={{...card,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div>
                  <p style={{margin:0,fontSize:14,fontWeight:700,color:T.textPri}}>{u.name}</p>
                  <p style={{margin:"2px 0 0",fontSize:12,color:T.textMut}}>{u.email} · {u.phone}</p>
                  <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
                    <GuardChip gIdx={u.gId}/>
                    <span style={{fontSize:10,color:T.textMut}}>Carnet: {u.cardNum}</span>
                    <span style={{fontSize:10,color:T.textMut}}>Posesión: {u.sd}</span>
                  </div>
                </div>
              </div>
      ...
      {adminTab==="balances"&&(
        <div>
          {/* User selector */}
          <div style={{marginBottom:16}}>
            <label style={{fontSize:12,color:T.textSec,fontWeight:600,display:"block",marginBottom:6}}>Seleccionar funcionario</label>
          ...
          {balUser&&(
            <div style={card}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
                ...
                <div>
                  <p style={{margin:0,fontWeight:700,fontSize:14,color:T.textPri}}>{balUser.name}</p>
                  <p style={{margin:0,fontSize:12,color:T.textSec}}>{yos(balUser.sd)} años de servicio · vacaciones base: {vacDays(yos(balUser.sd))} días</p>
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
                {AT.filter(a=>a.r).map(a=>{
                  ...
                  return (
                    <div key={a.id} style={{background:T.subBg,borderRadius:12,padding:"12px 14px",border:`1px solid ${T.subBrd}`}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <button onClick={()=>setUsers(us=>us.map(u=>u.id===balUser.id?{...u,bal:{...u.bal,[a.id]:Math.max(0,(u.bal?.[a.id]??0)-1)}}:u))}
                          style={{width:28,height:28,borderRadius:8,background:"rgba(239,68,68,0.15)",border:"none",cursor:"pointer",fontWeight:900,fontSize:16,color:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                        <span style={{fontSize:20,fontWeight:900,color:T.textPri,minWidth:28,textAlign:"center"}}>{cur}</span>
                        <button onClick={()=>setUsers(us=>us.map(u=>u.id===balUser.id?{...u,bal:{...u.bal,[a.id]:(u.bal?.[a.id]??0)+1}}:u))}
                          style={{width:28,height:28,borderRadius:8,background:"rgba(34,197,94,0.15)",border:"none",cursor:"pointer",fontWeight:900,fontSize:16,color:"#22c55e",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                        <span style={{fontSize:11,color:T.textMut,marginLeft:4}}>{used} usados</span>
                      </div>
                    </div>
                  );
                })}
          ...
          {!balUser&&<div style={{textAlign:"center",padding:"40px 0",color:T.textMut,fontSize:13}}>Selecciona un funcionario para editar su saldo</div>}
        </div>
      )}

      {adminTab==="extra"&&(
        <div>
          <ExtraShiftForm users={users} isMobile={isMobile} inp={inp} T={T} onAdd={e=>{setExtraShifts(es=>[...es,e]);addNotif(e.userId,`📋 El administrador te ha asignado un turno extra ${e.shift} el día ${e.date} (comp: ${e.compensation==="economic"?"económica":"días libres"})`)}}/>
          {extraShifts.map(e=>{
            const u=users.find(x=>x.id===e.userId);
            return (
              <div key={e.id} style={{...card,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6,padding:"10px 14px"}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  <ShiftChip shift={e.shift} small/>
                  <span style={{fontSize:13,fontWeight:600,color:T.textPri}}>{u?.name}</span>
                  <span style={{fontSize:12,color:T.textMut}}>{e.date}</span>
      ...
      {adminTab==="supplements"&&(
        <div>
          <SupplementForm isMobile={isMobile} inp={inp} T={T} onAdd={s=>setSupplements(ss=>[...ss,s])}/>
          {supplements.map(s=>(
            <div key={s.id} style={{...card,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6,padding:"10px 14px"}}>
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                <ShiftChip shift={s.shift} small/>
                <span style={{fontSize:13,fontWeight:600,color:T.textPri}}>{s.date}</span>
                <span style={{fontSize:14,fontWeight:700,color:"#22c55e"}}>+{s.amount}€</span>
                {s.note&&<span style={{fontSize:12,color:T.textMut}}>{s.note}</span>}
              </div>
  ...
  const renderNotifs = () => (
    <div>
      ...
      {myNotifs.length===0?(
        <div style={{textAlign:"center",padding:"50px 0",color:T.textMut,fontSize:14}}>Sin notificaciones</div>
      ):myNotifs.map(n=>(
        <div key={n.id} style={{...card,marginBottom:8,padding:"12px 16px",
          background:n.read?T.cardBg:T.subBg,
          borderLeft:`3px solid ${n.read?T.cardBrd:"#3b82f6"}`}}>
          <p style={{margin:0,fontSize:13,color:T.textPri}}>{n.text}</p>
          <p style={{margin:"4px 0 0",fontSize:11,color:T.textMut}}>{n.ts.toLocaleString("es-ES")}</p>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
        ...
        <div style={sidebar}>
          <div style={{padding:"20px 16px 14px"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div>
                ...
                <p style={{margin:0,color:T.navText,fontSize:10}}>Gestión de Guardias</p>
              </div>
            </div>
          </div>
            ...
            {navItems.map(item=>{
              ...
              return (
                <button key={item.id} onClick={()=>setView(item.id)}
                  ...
                  <span style={{fontSize:12,fontWeight:600,color:active?"white":T.navText,flex:1}}>{item.lbl}</span>
          ...
          <div style={{padding:"8px 8px 16px",display:"flex",flexDirection:"column",gap:6}}>
            <button onClick={toggleDark}
              ...
              <span style={{fontSize:12,color:T.navText,fontWeight:600}}>{darkMode ? 'Modo día' : 'Modo noche'}</span>
            </button>
            <button onClick={onLogout}
              style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:10,
                border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",cursor:"pointer"}}>
              <span style={{fontSize:13}}>🚪</span>
              <span style={{fontSize:12,color:T.navText,fontWeight:600}}>Cerrar sesión</span>
            </button>
          </div>
        </div>
      )}
      ...
      <div style={{flex:1,overflowY:"auto",background:T.pageBg,paddingBottom:isMobile?72:0}}>
        ...
        {/* Mobile top bar */}
        {isMobile&&(
          <div style={{background:T.navBg,padding:"12px 16px",
            ...
            return (
              <button key={item.id} onClick={()=>setView(item.id)}
                ...
                <span style={{fontSize:10,fontWeight:active?700:500,
                  color:active?T.accent:T.navText,marginTop:3,whiteSpace:"nowrap",
      ...
      {swapModal&&(
        <SwapDetailModal
          swap={swapModal.swap} users={users} currentUserId={currentUser.id} T={T}
      ...
      {guardModal&&(
        <GuardShiftModal date={guardModal.date} shift={guardModal.shift} gIdx={guardModal.gIdx}
          ...
          extraShifts={extraShifts} supplements={supplements} T={T}
          onClose={()=>setGuardModal(null)}/>
      )}
      {genModal&&(
        <GeneralModal date={genModal.date} shift={genModal.shift}
          users={users} absences={absences} swaps={swaps} T={T} onClose={()=>setGenModal(null)}/>
      )}
